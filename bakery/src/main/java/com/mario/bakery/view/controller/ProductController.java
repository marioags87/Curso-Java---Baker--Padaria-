...
public class ProductController {
  ...
  @PutMapping("/{id}")
  public ResponseEntity<ResponseRecord> update(
    @PathVariable Long id,
    @RequestBody ProductRecord productRecord
  ) {
    return service
      .findById(id)
      .map(entity -> {
        try {
          ...
          return ResponseEntity
            ...
            .body(new ResponseRecord(product, ""));
        } catch (ResponseException e) {
          return ResponseEntity
            .status(HttpStatus.BAD_REQUEST)
            .body(new ResponseRecord(null, e.getMessage()));
        }
      })
      .orElseGet(() ->
        ResponseEntity
          .status(HttpStatus.NOT_FOUND)
          .body(new ResponseRecord(null, "Product not found"))
      );
  }
